# Accommodation PHP SDK



The PHP SDK for the Accommodation API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Accommodation()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($accommodations as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $accommodation));
```
...
```php
require_once 'accommodation_sdk.php';
$client = new AccommodationSDK($options);
```
...
```php
$client = AccommodationSDK::test($testopts, $sdkopts);
```
...
```
php/
├── accommodation_sdk.php          -- Main SDK class
...
The main class (`accommodation_sdk.php`) exports the SDK class
...
// $accommodation->data_get() now returns the accommodation data from the last list
// $accommodation->match_get() returns the last match criteria
```
