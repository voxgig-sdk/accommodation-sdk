...
class AccommodationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Accommodation(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = AccommodationSDK::test($seed, null);
        $seen = iterator_to_array($base->Accommodation(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Accommodation(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = accommodation_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "accommodation." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ACCOMMODATION_TEST_ACCOMMODATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $accommodation_ref01_data = null;
        if (count($accommodation_ref01_data_raw) > 0) {
...
function accommodation_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/accommodation/AccommodationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["accommodation01", "accommodation02", "accommodation03"] as $k) {
    ...
    $entid_env_raw = getenv("ACCOMMODATION_TEST_ACCOMMODATION_ENTID");
    ...
    if ($env["ACCOMMODATION_TEST_LIVE"] === "TRUE") {
        ...
        $client = new AccommodationSDK(Helpers::to_map($merged_opts));
    }
