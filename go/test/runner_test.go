...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ACCOMMODATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("ACCOMMODATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ACCOMMODATION_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AccommodationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AccommodationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AccommodationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AccommodationError{Msg: msg, Code: code}
}
