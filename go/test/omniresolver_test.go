...
import (
	...
	omni "github.com/voxgig-sdk/accommodation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AccommodationError); is {
				out := map[string]any{
					"name":    "AccommodationError",
...
func omniCtx(arg any, client *sdk.AccommodationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AccommodationSDK); is {
			client = live
