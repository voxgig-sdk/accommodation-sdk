...
class AccommodationEntity
  ...
  def make
    opts = @_entopts.dup
    AccommodationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AccommodationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AccommodationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [AccommodationListMatch, Hash, nil] match filter (any subset of
  #   Accommodation fields); defaults to nil, treated as an empty match that lists all.
