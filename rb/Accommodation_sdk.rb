...
require_relative 'Accommodation_types'
...
class AccommodationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AccommodationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AccommodationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AccommodationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AccommodationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AccommodationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AccommodationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AccommodationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = AccommodationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AccommodationError.new(
        "graphql_error", "AccommodationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AccommodationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
