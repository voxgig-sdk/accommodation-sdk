...
boru-driven command-line client **and** interactive REPL for the Accommodation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/accommodation-cli)
make build

# 2. See usage (words, entities, env vars)
./accommodation-cli --help

# 3. Provide credentials once, via the environment
export ACCOMMODATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./accommodation-cli list accommodation

# 5. Override the API base URL for a single call
ACCOMMODATION_BASE=https://api.example.com ./accommodation-cli list accommodation
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/accommodation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/accommodation-cli list accommodation
   ```
...
Run with no arguments to open a REPL (prompt `accommodation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `accommodation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/accommodation-cli`. |
...
(`./accommodation-cli <expr>`) and interactively (the REPL), and expressions compose the
