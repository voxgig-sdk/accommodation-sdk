...
// Accommodation SDK retry feature
...
class AccommodationRetryFeature extends AccommodationBaseFeature
{
    ...
    public function init(AccommodationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AccommodationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AccommodationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AccommodationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
