...
// Accommodation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AccommodationOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AccommodationOmni::structfix($now, Struct::undef()))
                        !== json_encode(AccommodationOmni::structfix($val, Struct::undef()))) {
