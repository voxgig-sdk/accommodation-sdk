...
// Accommodation SDK feature test
//
...
class FtCtrl extends AccommodationControl
{
...
class FtClient
{
    ...
    private AccommodationUtility $utility;

    public function __construct(AccommodationUtility $utility, array $options)
    {
    ...
    public function get_utility(): AccommodationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AccommodationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AccommodationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AccommodationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AccommodationConfig::make_feature($name);
    ...
    private static function buildUrl(AccommodationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AccommodationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AccommodationResponse($fetched);
        }
    ...
    private function fail(AccommodationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AccommodationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
