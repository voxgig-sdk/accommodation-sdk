...
module AccommodationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AccommodationResponse.new({})
    ...
    if fetch_err
      ...
      response = AccommodationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AccommodationResponse.new(fetched)
    else
