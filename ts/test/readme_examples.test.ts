...
//     (\`new AccommodationSDK(...)\` / \`AccommodationSDK.test(...)\` become
//     \`AccommodationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AccommodationSDK'
...
const TEST_SEED = {"entity":{"accommodation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AccommodationSDK(...)\` and any
// \`AccommodationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AccommodationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AccommodationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
