...
describe('AccommodationDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AccommodationSDK({
  ...
  test('direct-list-accommodation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-accommodation', setup.live)) return
