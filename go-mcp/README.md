...
[MCP](https://modelcontextprotocol.io) server exposing the Accommodation SDK as
two agent tools — `accommodation_list` and `accommodation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/accommodation-mcp)
make build

# 2. Provide credentials via the environment
export ACCOMMODATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user accommodation \
  -- /absolute/path/to/accommodation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./accommodation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// accommodation_list: first page of records
{ "entity": "accommodation" }
{ "entity": "accommodation", "query": { } }

// accommodation_load: one record by id
{ "entity": "accommodation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/accommodation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user accommodation \
     -- "$PWD"/dist/*/accommodation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `accommodation_list` and `accommodation_load` tools now appear
   in new sessions. Ask the agent to *"list accommodation using accommodation"*
   and it calls `accommodation_list` with `{"entity":"accommodation"}`.
...
```sh
export ACCOMMODATION_APIKEY=sk_live_xxx            # API key
export ACCOMMODATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `accommodation_list` tool
...
### Call the `accommodation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `accommodation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `accommodation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ACCOMMODATION_APIKEY` | API key sent with every request. |
| `ACCOMMODATION_BASE` | Optional override of the API base URL. |
...
```sh
./accommodation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"accommodation_load","arguments":{"entity":"accommodation","query":{"id":1}}}}'
```
