...
type Args struct {
	Entity string         `json:"entity" jsonschema:"accommodation"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AccommodationSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "accommodation_list",
		Description: "List records from Accommodation. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "accommodation_load",
		Description: "Load a single record from Accommodation. " +
...
func runOp(client *sdk.AccommodationSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AccommodationSDK, name string) (sdk.AccommodationEntity, error) {
	switch strings.ToLower(name) {
	case "accommodation":
		return client.Accommodation(nil), nil
